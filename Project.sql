...
-- Policy status
create view current_status as select policy_end_date, case
when year(policy_end_date) = year(curdate()) then "Expired"
else "Running"
end as policy_status from policy_details;
select policy_status, count(*) from current_status group by policy_status;
...
-- growth rate
WITH policy_details AS (SELECT policy_start_date, Premium_Amount,
LAG(Premium_Amount) OVER (ORDER BY year(Policy_Start_Date)) AS previous_premiums FROM policy_details)
SELECT Policy_Start_Date, Premium_Amount, concat(round(((Premium_Amount - previous_premiums) / previous_premiums) * 100, 2), "%") AS growth_rate_percentage
FROM policy_details;
...
select product_group, count(opportunity_id) as oppty_count from opportunity group by product_group order by oppty_count desc;
